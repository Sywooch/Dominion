...
foreach ($_REQUEST as $param=>$val)
 {
  if(preg_match('/ATTR_(.)_(.+)/',$param,$m))
  {
   ...
   {
   $val=trim($val);
   if($tbl==1)  $val=preg_replace('/,/','.',$val);
   if($tbl>2 && $tbl!=7) $tbl=0;
   if($tbl==2 || $tbl==7) $cmf->execute('insert into ITEM'.$tbl.' (ITEM_ID,ATTRIBUT_ID,VALUE) values(?,?,?)',$_REQUEST['id'],$attr,$val);
   elseif($tbl==7) $cmf->execute('insert into ITEM64K (ITEM_ID,ATTRIBUT_ID,VALUE) values(?,?,?)',$_REQUEST['id'],$attr,$val);
   else $cmf->execute('insert into ITEM'.$tbl.'(ITEM_ID,ATTRIBUT_ID,VALUE) values(?,?,?)',$_REQUEST['id'],$attr,$val);
   }
  }
 }
}
