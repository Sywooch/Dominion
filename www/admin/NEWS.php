...
$cmf->execute('insert into NEWS (NEWS_ID,NEWS_GROUP_ID,DATA,NAME,CATNAME,DESCRIPTION,URL,IMAGE1,STATUS) values (?,?,?,?,?,?,?,?,?)',$_REQUEST['id'],stripslashes($_REQUEST['NEWS_GROUP_ID'])+0,stripslashes($_REQUEST['DATA']),stripslashes($_REQUEST['NAME']),stripslashes($_REQUEST['CATNAME']),stripslashes($_REQUEST['DESCRIPTION']),stripslashes($_REQUEST['URL']),stripslashes($_REQUEST['IMAGE1']),stripslashes($_REQUEST['STATUS']));
...
$cmf->execute('update NEWS set NEWS_GROUP_ID=?,DATA=?,NAME=?,CATNAME=?,DESCRIPTION=?,URL=?,IMAGE1=?,STATUS=? where NEWS_ID=?',stripslashes($_REQUEST['NEWS_GROUP_ID'])+0,stripslashes($_REQUEST['DATA']),stripslashes($_REQUEST['NAME']),stripslashes($_REQUEST['CATNAME']),stripslashes($_REQUEST['DESCRIPTION']),stripslashes($_REQUEST['URL']),stripslashes($_REQUEST['IMAGE1']),stripslashes($_REQUEST['STATUS']),$_REQUEST['id']);
$_REQUEST['e']='ED';
...
if($_REQUEST['e'] == 'ED')
{
list($V_NEWS_ID,$V_NEWS_GROUP_ID,$V_DATA,$V_NAME,$V_CATNAME,$V_DESCRIPTION,$V_URL,$V_IMAGE1,$V_STATUS)=
$cmf->selectrow_arrayQ('select NEWS_ID,NEWS_GROUP_ID,DATE_FORMAT(DATA,"%Y-%m-%d %H:%i"),NAME,CATNAME,DESCRIPTION,URL,IMAGE1,STATUS from NEWS where NEWS_ID=?',$_REQUEST['id']);



$V_STR_NEWS_GROUP_ID=$cmf->Spravotchnik($V_NEWS_GROUP_ID,'select NEWS_GROUP_ID,NAME from NEWS_GROUP  order by NAME');        
...
<form method="POST" action="NEWS.php" ENCTYPE="multipart/form-data" onsubmit="return true  &amp;&amp; checkXML(NAME) &amp;&amp; checkXML(CATNAME) &amp;&amp; checkXML(DESCRIPTION) &amp;&amp; checkXML(URL);">
...
<tr bgcolor="#FFFFFF"><th width="1%"><b>Группа новостей:<br /><img src="img/hi.gif" width="125" height="1" /></b></th><td width="100%">

	
			<select name="NEWS_GROUP_ID">
...
if($_REQUEST['e'] == 'Новый')
{
list($V_NEWS_ID,$V_NEWS_GROUP_ID,$V_DATA,$V_NAME,$V_CATNAME,$V_DESCRIPTION,$V_URL,$V_IMAGE1,$V_STATUS)=array('','','','','','','','','');
...
$sth=$cmf->execute('select A.NEWS_ID,A.NEWS_GROUP_ID,DATE_FORMAT(A.DATA,"%Y-%m-%d %H:%i"),A.NAME,A.STATUS from NEWS A where 1'.' order by A.DATA desc limit ?,?',$pagesize*($_REQUEST['p']-1),$pagesize);
...
print <<<EOF
<tr bgcolor="#FFFFFF"><td><input type="checkbox" onclick="return SelectAll(this.form,checked,'id[]');" /></td><th>N</th><th>Группа новостей</th><th>Дата</th><th>Заголовок новости</th><td></td></tr>
 
EOF;

if(is_resource($sth))
while(list($V_NEWS_ID,$V_NEWS_GROUP_ID,$V_DATA,$V_NAME,$V_STATUS)=mysql_fetch_array($sth, MYSQL_NUM))
{
$V_NEWS_GROUP_ID=$cmf->selectrow_arrayQ('select NAME from NEWS_GROUP where NEWS_GROUP_ID=?',$V_NEWS_GROUP_ID);
...
<td>$V_NEWS_ID</td><td>$V_NEWS_GROUP_ID</td><td>$V_DATA</td><td>$V_NAME</td><td nowrap="">
