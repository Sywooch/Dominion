...
//     }
   ...
   public function getDopparam($attribut_id, $item_id){
      ...
      elseif ($TYPE==2)  {$TABLE='ATTRIBUT_LIST'; $sel='attrl';}
      elseif ($TYPE==3)  {$TABLE='ATTRIBUT_LIST'; $sel='attr';}
      elseif ($TYPE==0 && !$IS_RANGEABLE )  {$TABLE='ATTRIBUT_LIST'; $sel='attr0';}
      elseif ($TYPE==1 && !$IS_RANGEABLE )  {$TABLE='ATTRIBUT_LIST'; $sel='attr1';}
      elseif ($TYPE<2 && $IS_RANGEABLE )  {$TABLE='RANGE_LIST'; $sel='attrr';}
   ...
   public function getAttributByCode($code){ 
...
//     echo $query->getQuery()."\r\n";
   ...
   public function getAttributByName($name, $type){ 
     $sql="select ATTRIBUT_ID
           from ATTRIBUT
           where NAME = '{$name}'
             and TYPE = {$type}";
   ...
   public function getAttributGroupByName($name){ 
   ...
   public function getUniteByName($name){ 
     $sql="select UNIT_ID
           from UNIT
           where NAME = '{$name}'";
   ...
   public function hasAttributList($name, $attribute_id){ 
   ...
   public function insertAttributList($data){ 
     $this->_db->insert('ATTRIBUT_LIST', $data);
           
     return $this->_db->lastInsertId();
   }
   
   public function getAttributOrder($attribut_group_id){ 
     $sql="select max(ORDERING)
           from ATTRIBUT
           where ATTRIBUT_GROUP_ID = ?";
           
     $ordering = $this->_db->fetchOne($sql, $attribut_group_id);  
     $ordering++;
     
     return $ordering;
   }
   
   public function insertAttribut($data){ 
...
//     $this->_db->insert('ATTRIBUT', $data);
   ...
   public function getAttributGroupOrder(){ 
     $sql="select max(ORDERING)
           from ATTRIBUT_GROUP";
           
     $ordering = $this->_db->fetchOne($sql);  
     $ordering++;
     
     return $ordering;
   }
   
   public function insertAttributGroup($data){ 
     $sql="insert into  ATTRIBUT_GROUP
           set NAME = '{$data['NAME']}'
