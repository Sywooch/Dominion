  ...
  class models_AnotherPages extends ZendDBEntity{
     ...
     public function getTree($parentId=0){
       ...
       for($i=0;$i<count($menu);$i++){
          ...
          if($children) $menu[$i]['menu_children']=$children;
       }
     ...
     public function getDocName($id){       
       return $this->_db->fetchOne("select NAME from ANOTHER_PAGES where ANOTHER_PAGES_ID = ?", $id);
     }
     ...
     public function getPageInfo($id,$lang){
       ...
       $info = $this->_db->fetchRow($sql);
        ...
        //XML
       $xml = $this->_db->fetchOne("select XML from XMLS where TYPE=0 and XMLS_ID=?",array($id));
     ...
     public function getSubmenu($id, $lang)
     {
        ...
        if($children)
        {
           ...
           {
               $info = $this->getDocInfo($children[$i]['ANOTHER_PAGES_ID'],$lang);
               if($info) $submenu[] = $info;
           }
        }
        return $submenu;
     }
     
    public function getDocXml($id, $type){     
      $sql="select XML 
            from XMLS 
            where XMLS_ID=? 
              and TYPE=?";
