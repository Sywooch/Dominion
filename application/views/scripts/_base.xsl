...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <!--  Для вывода чистого XHTML использовать это декларацию! --><!--<xsl:output encoding="UTF-8" indent="yes" omit-xml-declaration="yes" method="xml"/>--><!--<xsl:output encoding="UTF-8" indent="yes" omit-xml-declaration="yes" method="html"/>-->
    ...
    <xsl:template match="pop_link">
        <a href="#" onclick="return win_popup('{@href}',{@width},{@height});">
            <xsl:apply-templates/>
        </a>
    </xsl:template>
    ...
    <xsl:template match="binary[@html]">
        <xsl:value-of select="." disable-output-escaping="yes"/>
    </xsl:template>
    <xsl:template match="ancor">
        <a name="{@name}"></a>
    </xsl:template>
    ...
    <xsl:template match="bull">
        <img src="imgs/btn-more-b.gif" width="12" height="8" border="0" hspace="5" alt=""/>
    </xsl:template>
        ...
        <table width="{@width}" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr>
                <xsl:if test="@align='right'">
                    <td>
                        <img src="/i/0.gif" width="10" height="1"/>
                        <br/>
                    </td>
                </xsl:if>
                <td>
                    ...
                    <xsl:choose>
                        <xsl:when test="@zoom = 'yes'">
                            <a href="#">
                                <xsl:attribute name="onclick">zoom('<xsl:value-of select="@src1"/>');return false;
                                </xsl:attribute>
    ...
    <xsl:template match="picture[@border]">
        <table width="{@width}" border="0" align="left" cellpadding="0" cellspacing="0">
                                ...
                                <xsl:choose>
                                    <xsl:when test="@zoom = 'yes'">
                                        <a href="#">
                                            ...
                                            <img src="/images{@src}" width="{@width}" height="{@height}"
                                                 border="0"></img>
                                        </a>
                                    </xsl:when>
                                    <xsl:otherwise>
            ...
            <tr>
                <td colspan="2">
                    <img src="/i/0.gif" width="1" height="10"/>
                </td>
            </tr>
        </table>
    </xsl:template>
    ...
    <xsl:template match="picture[@href]">
        <a href="{@href}">
            ...
            <img src="/images/{@src}" width="{@width}" height="{@height}" border="0">
                <xsl:if test="@align">
                    <xsl:attribute name="align">
                        <xsl:value-of select="@align"/>
                    </xsl:attribute>
    ...
    <xsl:template match="flash">
        <object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000"
                ...
                width="{@width}" height="{@height}" id="flash" align="middle">
            <param name="allowScriptAccess" value="sameDomain"/>
            <param name="movie" value="/images/{@src}"/>
            <param name="quality" value="high"/>
            <param name="bgcolor" value="#f7f7f1"/>
            <param name="menu" value="false"/>
            <embed src="/images/{@src}" quality="high" bgcolor="#f7f7f1" width="{@width}" height="{@height}"
    ...
    <xsl:template match="embed">
        <embed src="{@src}" wmode="" quality="high" menu="false"
    ...
    <xsl:template match="footnote">
        ...
        <xsl:text
                disable-output-escaping="yes">&lt;</xsl:text>a href='#' class='footnote' onclick='return
        showBk(<xsl:value-of
            select="$id"/>,
        <xsl:text disable-output-escaping="yes">"</xsl:text>
                ...
                <xsl:when test="$cur != $current">
                    ...
                    <xsl:choose>
                        <xsl:when test="$cur!=1">
                            ...
                            <xsl:call-template name="show_digit_link_page_active">
                                <xsl:with-param name="href">
                                    <xsl:value-of select="$url"/>page/<xsl:value-of select="$cur"/>/
                                </xsl:with-param>
                                <xsl:with-param name="link" select="$cur"/>
                            </xsl:call-template>
            ...
            <xsl:choose>
                <xsl:when test="@pcount &gt; 1">
                    ...
                    <xsl:if test="number(@pcount) &gt; 11 and $start_number &gt; 1">
                        ...
                        <xsl:choose>
                            <xsl:when test="(@page - 5) &lt;= 3">
                                <!-- Просто до писать недостающие страницы -->
                                <xsl:call-template name="section">
                                    <xsl:with-param name="current" select="@page"/>
                                    <xsl:with-param name="pages" select="$start_number - 1"/>
                                    <xsl:with-param name="cur" select="1"/>
                                </xsl:call-template>

                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:call-template name="first_pages"/>
                                ...
                                <xsl:call-template name="show_digit_link_page_active">
                                    <xsl:with-param name="href">
                                        ...
                                        <xsl:value-of select="$url"/>page/<xsl:value-of select="$prev_number"/>/
                                    </xsl:with-param>
                    ...
                    <!-- Вводим рекурсивно 11 ссылок на страницы -->
                    <xsl:call-template name="section">
                        <xsl:with-param name="current" select="@page"/>
                        <xsl:with-param name="pages" select="$pages"/>
                    ...
                    <xsl:choose>
                        <xsl:when test="(number(@pcount) - $pages) &lt;= 3">
                            <!-- довыводим недостающие страницы -->
                            <xsl:call-template name="section">
                                <xsl:with-param name="current" select="$start_number"/>
                                <xsl:with-param name="pages" select="@pcount"/>
                                <xsl:with-param name="cur" select="@page + 11"/>
                            </xsl:call-template>


                        </xsl:when>
                        <xsl:otherwise>
                            <!-- выводим троеточие и последние три страницы -->
                            <xsl:call-template name="show_digit_link_page_active">
                                <xsl:with-param name="href">
                                    ...
                                    <xsl:value-of select="$url"/>page/<xsl:value-of select="$next_number"/>/
                                </xsl:with-param>
                                <xsl:with-param name="link">&#8230;</xsl:with-param>
                            </xsl:call-template>

                            <!-- Последние 3 страницы -->
                            <xsl:call-template name="last_pages">
                    ...
                    <xsl:choose>
                        <xsl:when test="@page &lt; @pcount">
                            ...
                            <li class="next">
                                <a>
                                    <xsl:variable name="href">
                                        <xsl:value-of select="$url"/>page/<xsl:value-of select="@page+1"/>/
                                    </xsl:variable>
                                    <xsl:attribute name="href">
                                        <xsl:value-of select="translate(normalize-space($href), ' ', '')"/>
                                    </xsl:attribute>
    ...
    <xsl:template name="show_digit_link_page_active">
        <xsl:param name="href"/>
        <xsl:param name="link"/>
        <li>
            <a>
    ...
    <xsl:template name="first_pages">
        ...
        <xsl:call-template name="show_digit_link_page_active">
            <xsl:with-param name="href">
                <xsl:variable name="url">
                    <xsl:call-template name="section_url">
                        <xsl:with-param name="cur" select="2"/>
                    </xsl:call-template>
                </xsl:variable>
                <xsl:value-of select="$url"/>page/2/
            </xsl:with-param>
            <xsl:with-param name="link" select="2"/>
        </xsl:call-template>
        ...
        <xsl:call-template name="show_digit_link_page_active">
            <xsl:with-param name="href">
                <xsl:variable name="url">
                    <xsl:call-template name="section_url">
                        <xsl:with-param name="cur" select="3"/>
                    </xsl:call-template>
                </xsl:variable>
                <xsl:value-of select="$url"/>page/3/
            </xsl:with-param>
            <xsl:with-param name="link" select="3"/>
        </xsl:call-template>
    </xsl:template>


    <xsl:template name="last_pages">
        ...
        <xsl:call-template name="show_digit_link_page_active">
            <xsl:with-param name="href">
                <xsl:variable name="url">
                    <xsl:call-template name="section_url">
                        <xsl:with-param name="cur" select="$pcount - 2"/>
                    </xsl:call-template>
                </xsl:variable>
                <xsl:value-of select="$url"/>page/<xsl:value-of select="$pcount - 2"/>/
            </xsl:with-param>
            <xsl:with-param name="link" select="$pcount - 2"/>
        </xsl:call-template>
        ...
        <xsl:call-template name="show_digit_link_page_active">
            <xsl:with-param name="href">
                <xsl:variable name="url">
                    <xsl:call-template name="section_url">
                        <xsl:with-param name="cur" select="$pcount - 1"/>
                    </xsl:call-template>
                </xsl:variable>
                <xsl:value-of select="$url"/>page/<xsl:value-of select="$pcount - 1"/>/
            </xsl:with-param>
            <xsl:with-param name="link" select="$pcount - 1"/>
        </xsl:call-template>
        ...
        <xsl:call-template name="show_digit_link_page_active">
            <xsl:with-param name="href">
                <xsl:variable name="url">
                    <xsl:call-template name="section_url">
                        <xsl:with-param name="cur" select="$pcount"/>
                    </xsl:call-template>
                </xsl:variable>
                <xsl:value-of select="$url"/>page/<xsl:value-of select="$pcount"/>/
            </xsl:with-param>
            <xsl:with-param name="link" select="$pcount"/>
        </xsl:call-template>
        ...
        <xsl:choose>
                ...
                <xsl:choose>
                    <xsl:when test="image/w &gt; 0 and url != ''">
                        <a href="{url}" title="{alt}">
                            ...
                            <img src="/images/banners/{image/src}" width="{image/w}" height="{image/h}" alt="{alt}"
                                 title="{alt}" border="0"/>
                        </a>
                    </xsl:when>
            ...
            <xsl:when test="type = '1'"> <!-- Flash -->
                ...
                <object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"
                    ...
                    <embed src="/images/banners/{image/src}" menu="false" quality="high" devicefont="true"
                           bgcolor="#FFFFFF" type="application/x-shockwave-flash"
                           pluginspage="http://www.macromedia.com/go/getflashplayer" width="{$width}"
                           height="{$height}"/>
                </object>
            </xsl:when>
