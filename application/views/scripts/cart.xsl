...
<xsl:template match="payment">
  <option value="{@id}">
	  <xsl:if test="@id=/page/payment_err"><xsl:attribute name="selected">selected</xsl:attribute></xsl:if>
		...
		<xsl:when test="count(item) &gt; 0">
			<div id="content_cart" class="chapter_products content_block">
				...
				<div id="cart_total" class="clearfix">
					<span>Всего к оплате <span class="price"><xsl:value-of select="format-number(total_summ, '### ##0', 'european')"/></span>&#160;<xsl:value-of select="sname"/></span>
					<div>
						<a class="product_button" href="#"><span><span>Оформить заказ</span></span></a>
			...
			<div id="cart_text_block" class="clearfix content_block">
				...
				<div class="right">
					<form method="post" action="/cart/order/" id="cart_order_form" class="text_block_form">
						...
						<p>
							<i>Все поля, отмеченные <span class="asterisk">*</span>, обязательны для заполнения</i>
						</p>
						<table>
							<tbody>
								<tr>
								...
								<tr>
									<td>
										<label for="name">Фамилия и имя<span class="asterisk">*</span></label>
								...
								<tr>
									<td>
										...
										<input id="email" name="email" type="email" value="{//user_data/user_email}"/>
									</td>
								</tr>
								<tr>
									<td>
										<label for="telmob">Телефон:<span class="asterisk">*</span></label>
										<input id="telmob" name="telmob" type="text" value="{//user_data/user_phone}"/>
									</td>
								</tr>
								<tr>
									<td>
								...
								<tr>
									<td>
										<label for="more_information">Дополнительная информация</label>
										<textarea id="more_information" name="info"></textarea>
									</td>
								</tr>
							</tbody>
						</table>
						<a class="product_button" href="#" id="order_button"><span><span>Подтвердить заказ</span></span></a>
					</form>
				</div>
			</div>
		</xsl:when>
