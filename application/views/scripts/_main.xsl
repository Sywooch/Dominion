...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="socials">
		<li><a href="{url}" target="_blank"><img src="/images/social/{image/@src}" width="{image/@w}" height="{image/@h}" /></a></li>
	</xsl:template>
	...
	<xsl:template name="through-brands">
		<xsl:choose>
			...
			<xsl:otherwise><h3>Популярные производители</h3></xsl:otherwise>
		</xsl:choose>
		...
		<xsl:choose>
			<xsl:when test="not(//data/@section)"></xsl:when>
			<xsl:otherwise><ul class="brands"></ul></xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="cart">
		<xsl:choose>
			<xsl:when test="//cart/total_count &gt; 0">
				<a href="/cart/">Корзина
				<br />
				<span>Товаров <xsl:value-of select="//cart/total_count"/> &mdash; <xsl:value-of select="//cart/total_summ"/> грн</span></a>
			</xsl:when>
			<xsl:otherwise>
				<a>Корзина
				<br />
				<span>Товаров 0</span></a>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="login_form">
		<a href="#" class="pseudo"><span>Вход</span></a>
					...
					<div id="header">
						...
						<form id="search_form" method="get" action="/search/">
							<input id="search_text" name="search_text" type="text" value="Поиск по сайту">
								<xsl:if test="data/query != '' ">
									<xsl:attribute name="value"><xsl:value-of select="data/query"/></xsl:attribute>
								</xsl:if>
							</input>
						...
						<div id="header_phones">
							<div class="phone">
								...
								<a href="#" class="pseudo"><span><xsl:apply-templates select="//right_side_mobile"/></span></a>
							...
							<div class="phone">
								...
								<strong><xsl:apply-templates select="//right_side_phone"/></strong>
							</div>
							<div class="last">
								<a href="#" class="phone_icon">Phone number</a>
								<a href="#" class="pseudo"><span>Заказать обратный звонок</span></a>
								<div id="call_form_box" class="dialog_box call">
									<a href="#" class="close_icon">Close</a>
									<form id="call_form" action="/ajax/getcall/" method="post" class="dialog_content">
										<div class="errhold"></div>
										<div>
											<label for="call_name">ФИО<span class="asterisk">*</span>:</label>
											<input id="call_name" name="call_name" type="text"/>
										</div>
										<div>
											<label for="call_phone">Телефон для связи<span class="asterisk">*</span>:</label>
				...
				<div id="footer_carrier">
					<div id="footer">
						...
						<div id="copy">
							...
							<span class="develop"><p>
								<a href="/ajax/go/?url=http://adlabs.com.ua">Разработка сайта &mdash; AD|LABS</a></p>
							</span>
						</div>
	...
	<xsl:template match="data[not(@is_start)]" mode="body">
		<div id="content">
			<xsl:if test="count(breadcrumbs) &gt; 0">
				<ul class="breadcrumb">
					<li>
						<a href="/" class="pseudo"><span>Главная</span></a>
					</li>
