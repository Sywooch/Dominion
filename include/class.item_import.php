<?php
  ...
  class item_import {
    ...
    public function loadXMLFile($file_name) {
      try {
        if (!is_file($file_name))
          throw new ExceptionEmptyPath('File ' . $file_name . ' is not exist');

        $content = file_get_contents($file_name);

        if (empty($content))
          throw new ExceptionEmptyBuffer('XML data is empty');
    ...
    private function setCurrency() {
      ...
      try {
        if ($entries->length == 0)
          throw new ExceptionEmptyBuffer('Node currency is not exist');
        ...
        if (empty($code))
          throw new ExceptionEmptyBuffer('Node currency is empty');
    ...
    */
    ...
    private function setOffers() {
      ...
      try {
        if ($entries->length == 0)
          throw new ExceptionEmptyBuffer('Node offers is not exist');
      ...
      foreach ($params as $val) {
        ...
        if (!empty($atribut_value)) {
          ...
          if ($_get_attrib === false)
            throw new ExceptionEmptyBuffer('Param ' . $atribut_id . ' is absent in params');
